...
public class MealOrderHelper {
    ...
    public JSONObject getByTicketId(int id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `missa`.`meal_order` WHERE `ticket_id`= ? ";
    ...
    public JSONObject getByIdList(String data) {
      /** 新建一個 Meal 物件之 m 變數，用於紀錄每一位查詢回之商品資料 */
      Meal m = null;
      /** 用於儲存所有檢索回之商品，以JSONArray方式儲存 */
      JSONArray jsa = new JSONArray();
      /** 記錄實際執行之SQL指令 */
      ...
      try {
          ...
          /** SQL指令 */
          String sql = "SELECT * FROM `missa`.`meal` WHERE `meal`.`meal_id`";
    ...
    public JSONObject create(int ticket_id,String meal_id, int  quantity) {
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
    ...
    public JSONObject update(Meal m) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
